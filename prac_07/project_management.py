...
def update_projects(projects):
    ...
    try:
        ...
        if not project_choice <0 or project_choice >= len(projects):
            ...
            if not len(new_percentage.strip()) == 0:
                if not int(new_percentage) < MINIMUM_VALUE or int(new_percentage) > MAXIMUM_VALUE:
                    projects[project_choice].completed_percentage=int(new_percentage)
            new_priority=input("new priority:")
            if not len(new_priority.strip()) == 0:
                if not int(new_priority) < MINIMUM_VALUE+1 or int(new_priority) > MAXIMUM_VALUE:
...
def get_valid_number(prompt,number_type):
    ...
    while not is_valid:
        try:
            number=number_type(input(f"{prompt}"))
            is_valid=True
        except ValueError:
...
def get_valid_date(prompt):
    ...
    is_valid = False
    date_object = 0 #Set the initial value to 0
    while not is_valid:
        date_string = input(f"{prompt}: ")
        try:
...
def filter_project(projects):
    ...
    filer_date=get_valid_date("Show projects that start after date (dd/mm/yyyy)")
    filter_dates=[project for project in projects if project.start_date >= filer_date]
    filter_dates.sort(key=attrgetter('start_date'))
...
def save_projects(projects):
    ...
    with open(filename,"w") as out_file:
        ...
        for project in projects:
            formatted_date = project.start_date.strftime(DATE_FORMAT)
            print("\t".join([project.name,formatted_date,str(project.priority),
